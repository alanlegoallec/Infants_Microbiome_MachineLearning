...
metricsS <- list("age_at_collection"=c("R2"), "Surv"="CI", "country"= c("Cross_Entropy", "Mean_Accuracy", "Accuracy", "FIN", "EST", "RUS", "SWE" ), "HvsFDvsD"=c("Cross_Entropy", "Mean_Accuracy", "Accuracy", "H", "FD", "D"))
...
families <- c("age_at_collection" = "gaussian", "abx_usage"="binomial", "exclusive_bf"="binomial", "delivery_type"="binomial", "sex"="binomial", "country"="multinomial", "HvsD"="binomial", "HvsFD"="binomial", "FDvsD"="binomial", "HvsFDD"="binomial", "HvsFDvsD"="multinomial", "Surv"="cox")
...
#Display variables
#define
analyses_labels <- c("Age", "Antibiotics Usage", "Exclusively Breastfed", "Delivery Type", "Sex", "Country of Origin", "Healthy vs Future Diabetic", "Healthy vs Diabetic", "Future Diabetic vs Diabetic" , "Healthy vs Future Diabetic/Diabetic", "Healthy vs Future Diabetic vs Diabetic", "Diabetes Onset")
...
metricsS_labels <- list("age_at_collection"=c("R-Squared"), "Surv"=c("Concordance Index"), "country"= c("Mean Class Accuracies", "Accuracy", "Finland", "Estonia", "Russia", "Sweden", "Cross Entropy"), "HvsFDvsD"=c("Mean Class Accuracies", "Accuracy", "Healthy", "Future Diabetic", "Diabetic", "Cross Entropy"))
for (ana in c("abx_usage", "exclusive_bf", "delivery_type", "sex", "HvsFD", "HvsD", "FDvsD", "HvsFDD")){metricsS_labels[[ana]] <- c("AUC of the ROC", "Mean Class Accuracies", "Accuracy", "Sensitivity", "Specificity",  "Cross Entropy")}
...
dictionary_analysis <- data.frame("name"=c("age_at_collection", "abx_usage", "exclusive_bf", "delivery_type", "sex", "country", "HvsFD", "HvsD", "FDvsD", "HvsFDD", "HvsFDvsD", "Surv"), "label" = c("Age", "Antibiotics Usage", "Exclusively Breastfed", "Delivery Type", "Sex", "Country of Origin", "Healthy vs Future Diabetic", "Healthy vs Diabetic", "Future Diabetic vs Diabetic", "Healthy vs Future Diabetic/Diabetic", "Healthy vs Future Diabetic vs Diabetic", "Diabetes Onset"), stringsAsFactors=FALSE)      
...
dictionary_metric <- data.frame("name"=c("R2", "CI", "ROC", "Cross_Entropy", "Mean_Accuracy", "Accuracy", "Sensitivity", "Specificity", "FIN", "EST", "RUS", "SWE", "H", "FD", "D", "X0", "X1"), "label"=c("R-Squared", "Concordance Index", "AUC of the ROC", "Cross Entropy", "Mean Class Accuracies", "Accuracy", "Sensitivity", "Specificity", "Finland", "Estonia", "Russia", "Sweden", "Healthy", "Future Diabetic", "Diabetic", "Class 0", "Class 1"), stringsAsFactors=FALSE)
dictionary_fold <- data.frame("name"=seq(0,N_CV_Folds), "label"=c("All samples", seq(N_CV_Folds)), stringsAsFactors=FALSE)
dictionary_multinomial <- data.frame("name"=c("H", "FD", "D", "FIN", "EST", "RUS", "SWE"), "label"=c("Healthy", "Future_Diabetic", "Diabetic", "Finland", "Estonia", "Russia", "Sweden"), stringsAsFactors=FALSE)
n_digits_display <- list("R2"=3, "CI"=3, "ROC"=3, "Cross_Entropy"=5, "Mean_Accuracy"=3,"Accuracy"=3, "Sensitivity"=3, "Specificity"=3, "FIN"=3, "EST"=3, "RUS"=3, "SWE"=3, "H"=3, "FD"=3, "D"=3)
...
display_performances <- list("R2"=T, "CI"=T, "ROC"=T, "Cross_Entropy"=F, "Mean_Accuracy"=T, "Accuracy"=F, "Sensitivity"=F, "Specificity"=F, "FIN"=F, "EST"=F, "RUS"=F, "SWE"=F, "H"=F, "FD"=F, "D"=F)
...
variables_to_remove_associations <- c()
if(app_version=="diabetes") #diabetes only app
{
  analyses_labels <- c("Healthy vs Future Diabetic", "Healthy vs Diabetic", "Future Diabetic vs Diabetic" , "Healthy vs Future Diabetic/Diabetic", "Healthy vs Future Diabetic vs Diabetic", "Diabetes Onset")
} else if (app_version=="other phenotypes") { #other phenotypes only app
...
{
  data = data[indices,]
  c <- concordance.index(x = data[["pred"]], surv.time= data[["time"]], surv.event= data[["status"]])$c.index
  return(c)
}
...
{
  ifelse(analysis=="Surv", lgth_w <- length(y)/2, lgth_w <- length(y))
  w <- seq(1,lgth_w)*0 + 1
  if(!(analysis %in% c("age_at_collection", "Surv")))
  {
    table_y <- table(y)
...
preprocessing_folds_0 <- function(analysis, target, predictors)
{
  ...
  if(analysis=="Surv")
  {
    ...
    time_train <- data_train$time_to_onset
    status_train <- data_train$seroconverted_ever
  } else {
    y <- data_train[,target]
  ...
  ifelse(analysis=="Surv", var_to_remove <- c(target, "time_to_onset"), var_to_remove <- target)
  x_train = as.matrix(data_train[,-which(names(data_train) %in% var_to_remove)])
  ifelse(analysis == "Surv", y_train <- Surv(data_train$time_to_onset, data_train$seroconverted_ever), y_train <- data_train[, target])
  if(analysis=="Surv")
  {
    preprocessed_data <- list("data_train" = data_train, "x_train" = x_train, "y_train" = y_train, "w_train" = w_train, "time_train" = time_train, "status_train" = status_train, "data_test" = data_train, "x_test" = x_train, "y_test" = y_train, "w_test" = w_train,  "time_test" = time_train, "status_test" = status_train)
  } else {
  ...
  if(analysis=="Surv"){preprocessed_data["time_train"] <- time_train}
...
preprocessing_folds <- function(analysis, target, predictors, i)
{
  ...
  #filter predictors
  COEFS <- initiate_store(names_predictors, c("coefficients", "p-values"))
  ifelse(analysis=="Surv", y <- Surv(data_train$time_to_onset, data_train$seroconverted_ever), y <- data_train[,target])
  ...
  if(analysis=="Surv")
  {
    time_train <- data_train$time_to_onset
    status_train <- data_train$seroconverted_ever
    y_train <- Surv(time_train, status_train)
  } else {
    y_train <- data_train[, target]
  }
  #test
  ...
  if(analysis=="Surv")
  {
    time_test <- data_test$time_to_onset
    status_test <- data_test$seroconverted_ever
  ...
  if(analysis=="Surv")
  {
    preprocessed_data <- list("data_train" = data_train, "x_train" = x_train, "y_train" = y_train, "w_train" = w_train, "time_train" = time_train, "status_train" = status_train, "data_test" = data_test, "x_test" = x_test, "y_test" = y_test, "w_test" = w_test, "time_test" = time_test, "status_test" = status_test)
  } else {
...
preprocessing_mixed <- function(analysis, i)
{
  for(set in c("train", "test"))
  {
    ...
    ifelse(analysis=="Surv", string <- "demo+glmnet2", string <- "demo+glmnet")
  ...
  print(table(COEFS$`p-values` < 0.05/nrow(COEFS)))
  print(table(COEFS$coefficients < 0))
  ifelse(analysis=="Surv", to_remove <- c(target, "time_to_onset"), to_remove <- target)
  data_train <- data_train[, c(names_others, index)]
  data_test <- data_test[, c(names_others, index)]
...
preprocessing_nodemo <- function(analysis, i)
{
  ...
  if(analysis=="Surv"){to_remove <- to_remove[which(!(to_remove=="time_to_onset"))]}
#  if(analysis=="Surv"){to_remove <- c(to_remove, target)}
  for (predictors in predictorsS_withdemo)
  {
    if (predictors == "demo+genes")
    {
      ifelse(analysis=="Surv", algos_genes_list <- algos_surv, algos_genes_list <- algos_genes)
...
boot_performance_HvsFDvsD <- function(data, indices)
{
  ...
  y <- as.character(y)
  Accuracy <- mean(y == pred_class)
  H <- sum(pred_class == "H" & y == "H")/table(y)["H"]
  FD <- sum(pred_class == "FD" & y == "FD") /table(y)["FD"]
  D <- sum(pred_class == "D" & y == "D") /table(y)["D"]
...
{
  ifelse(analysis == "country", classes_target <- c("FIN", "EST", "RUS", "SWE"), classes_target <- c("H", "FD", "D"))
  ifelse(predictors %in% c("genes", "demo+genes"), algos_list_pp <- algos_genes, algos_list_pp <- algos)
...
post_processing_surv <- function(analysis, predictors, set)
{
  Performance <- initiate_store(algos_surv, c("CI"))
  Performance[,] <- NA
  Performance_sd <- Performance
  for (algo in rownames(Performance))
  {
    ...
    time_var <- c()
    status_var <- c()
    Performance_algo <- initiate_store(c("all", seq(0,N_CV_Folds)) , c("CI"))
    Performance_algo[,] <- NA
    Performance_algo_sd <- Performance_algo
    for (i in seq(0,N_CV_Folds))
    {
      print(paste("Computing performance for fold ", i, ".", sep=""))
      pp_data_i <- readRDS(paste(path_compute, "preprocessed_data_", analysis, "_", string, "_", i, ".Rda", sep = ""))
      y_i <- pp_data_i[[paste("y", set, sep = "_")]]
      time_var_i <- pp_data_i[[paste("time", set, sep = "_")]]
      status_var_i <- pp_data_i[[paste("status", set, sep = "_")]]
      pred_i <- readRDS(paste(path_compute, "pred_", set, "_", analysis, "_", predictors, "_", algo, "_", i, ".Rda", sep = ""))
      index_not_NA <- which(!is.na(pred_i))
      y_i <- y_i[index_not_NA]
      time_var_i <- time_var_i[index_not_NA]
      status_var_i <- status_var_i[index_not_NA]
      w_i <- seq(length(y_i))*0+1
      pred_i <- pred_i[index_not_NA]
      if(!(i==0))
      {
        y <- c(y, y_i)
        pred <- c(pred, pred_i)
        time_var <- c(time_var, time_var_i)
        status_var <- c(status_var, status_var_i)
      }
      data_i <- data.frame("time"=time_var_i, "status"=status_var_i, "pred"=pred_i)
      data_i <- data_i[data_i$status==1,]
      perf_boot <- boot(data = data_i, statistic = boot_CI, R = R_boot)
      Performance_algo[as.character(i),] <- perf_boot$t0
      Performance_algo_sd[as.character(i),] <- sapply(data.frame(perf_boot$t), sd, na.rm=T)
      if(i==0){sample_sizes <- initiate_store(c("all", seq(0,N_CV_Folds)), c(paste("N", set, sep="_"), paste("N", set, levels(as.factor(status_var_i)), sep="_")))}
      sample_sizes[as.character(i),] <- c(length(status_var_i), table(status_var_i))
    }
    w <- generate_weights(y)
    data <- data.frame("time"=time_var, "status"=status_var, "pred"=pred)
    data <- data[data$status==1,]
    perf_boot <- boot(data = data.frame("time"=time_var, "status"=status_var, "pred"=pred), statistic = boot_CI, R = R_boot)
    mean_boot <- perf_boot$t0
    sd_boot <- sapply(data.frame(perf_boot$t), sd, na.rm=T)
    Performance_algo["all",] <- mean_boot
    Performance_algo_sd["all",] <- sd_boot
    sample_sizes["all",] <- c(length(status_var), table(status_var)[levels(as.factor(status_var_i))])
    Performance[algo,] <- mean_boot
    Performance_sd[algo,] <- sd_boot
    saveRDS(Performance_algo, paste(path_compute, "Performance_", set, "_", analysis, "_", predictors, "_", algo, ".Rda", sep = ""))
    saveRDS(Performance_algo_sd, paste(path_compute, "Performance_sd_", set, "_", analysis, "_", predictors, "_", algo, ".Rda", sep = ""))
...
testing <- function(analysis, target, predictors, algo, i, data, x, y, w, set)
{
  ...
  if (analysis == "Surv")
  {
    if(algo=="glmnet2")
    {
      ...
      pred <- do.call(predict, args=args_predict)[,1]
    } else if (algo=="gbm2") {
      pred <- do.call(predict, args=args_predict)
    } else if (algo == "rf2") {
      pred <- do.call(predict, args=args_predict)$predicted
    }
...
best_predictors <- function(model, analysis, predictors, algo, i)
{
  if (algo=="glmnet") {
    if(analysis %in% c("country", "HvsFDvsD"))
    {
      ...
      ifelse(analysis=="country", y_classes <- c("FIN", "EST", "RUS", "SWE"), y_classes <- c("H", "FD", "D"))
...
best_predictors_surv <- function(model, analysis, predictors, algo, i)
{
  if(algo=="glmnet2") {
    variables_importance <- data.frame(as.matrix(coef(model)))
    variables_importance <- variables_importance[order(abs(variables_importance[,1]),decreasing = TRUE ),, drop = FALSE]
    names(variables_importance) <- "Regression_Coefficients"
  } else if(algo=="gbm2") {
    variables_importance <- summary(model)[,2,drop=FALSE]
    ...
    variables_importance <- data.frame("Importances" = model$importance[order(model$importance, decreasing=TRUE)])
    names(variables_importance) <- c("Importances")
  }
  #print(head(variables_importance, 100))
  saveRDS(variables_importance, paste(path_store, "variables_importance_", analysis, "_", predictors, "_", algo, "_", i, ".Rda", sep = ""))
}
...
tune_caret_models <- function(analysis, target, predictors, algo, i, data, x, y, w)
{
  ...
  if(analysis %in% c("age_at_collection", "country", "HvsFDvsD") & algo %in% c("gbm", "svmPoly", "nb")){args_trctrl[["allowParallel"]] <- T}
...
tune_surv_glmnet2 <- function(analysis, target, predictors, algo, i, data, x, y, w)
{
  alpha <- 1 #default value for the package, corresponds to ridge regression.
  names_hyper <- c("lambda")
  strides_hyper <- list("lambda"=.5)
  ...
  if(analysis == "Surv") {
    metriC <- "deviance"
  } else if (analysis %in% c("age_at_collection", "country", "HvsFDvsD")) {
    metriC <- "mse"
  } else {
    metriC <- "auc"}
  flds <- createFolds(data$target, k = N_CV_Folds, list = TRUE, returnTrain = FALSE)
  foldids = rep(1,length(data$target))
  for (j in seq(N_CV_Folds)){foldids[flds[[paste("Fold",sprintf("%02d", j),sep="")]]] <- j}
  model <- cv.glmnet(x = x, y = y, weights = w, family = families[analysis], lambda = transformed_hyper[["lambda"]], alpha = alpha, type.measure = metriC, nfolds = N_CV_Folds, foldid = foldids, parallel = FALSE, standardize = FALSE)
  results <- glmnet2_results_fxn(model$lambda, model$cvm, metric)
  best_tune <- list("lambda"=model$lambda.min)
...
tune_surv_gbm2 <- function(analysis, target, predictors, algo, i, data, x, y, w, n_cores=n_cores, new_seed=seed)
{
  data <-data[,-which(names(data) %in% c("target", "time_to_onset"))]
  set.seed(new_seed) #change the seed when the model crashes because of a memory mapping issue.
  n_trees <- 1000
  print(paste("number of trees = ", n_trees, sep=""))
  error <- tryCatch(  model <- gbm(formula = y ~., data = data, distribution = "coxph", n.trees = n_trees, interaction.depth = 1, n.minobsinnode = 10, shrinkage = 0.1, cv.folds = N_CV_Folds, n.cores = n_cores), error=function(err) "error")
  while(grepl("error", error))
  {
    print(paste("The model crashed, changing the seed to ", (new_seed+1), sep=""))
    new_seed <- new_seed+1
    set.seed(new_seed)
    error <- tryCatch(model <- gbm(formula = y ~., data = data, distribution = "coxph", n.trees = n_trees, interaction.depth = 1, n.minobsinnode = 10, shrinkage = 0.1, cv.folds = N_CV_Folds, n.cores = n_cores), error=function(err) "error")
  }
  ...
  while(which.min(model$cv.error) > floor(0.9*n_trees) & (mean_89-mean_910)/mean_910 > 0.0001)
  {
    ...
    n_trees<- n_trees*2
    print(paste("number of trees = ", n_trees, sep=""))
    model <- gbm(formula = y ~., data = data, distribution = "coxph", n.trees = n_trees, interaction.depth = 1, n.minobsinnode = 10, shrinkage = 0.1, cv.folds = N_CV_Folds, n.cores = n_cores)
    #    print(model) #this line crashes if there is a NaN somewhere in model
    mean_89 <- mean(model$cv.error[ceiling(0.8*n_trees):floor(0.9*n_trees)])
    mean_910 <- mean(model$cv.error[ceiling(0.9*n_trees):n_trees])
  }
...
tune_surv_rf2 <- function(analysis, target, predictors, algo, i, data, x, y, w, n_cores=1)
{
  print("starting algo")
  options(rf.cores = n_cores)
  n_trees <- 1001
  parameters_rfsrc <- tune(Surv(time_to_onset, target) ~., ntreeTry = n_trees, data = data)
  print(parameters_rfsrc)
  model <- rfsrc(Surv(time_to_onset, target) ~., data = data, ntree = n_trees, nodesize = parameters_rfsrc$optimal["nodesize"], mtry=parameters_rfsrc$optimal["mtry"], importance = TRUE)
  saveRDS(c(model$mtry, model$nodesize), paste(path_compute, "hyperparameters", "_", analysis, "_", predictors, "_", algo, "_", i,".Rda", sep = ""))
  print("Results:"); print(model); print("Best tune:"); print(c(model$mtry, model$nodesize))
  return(model)
}
...
{
  names_demographics <- c("age_at_collection", "abx_usage", "exclusive_bf", "delivery_type", "sex", "country", "seroconverted_time", "seroconverted_ever", "D_status")
  ...
  levels(data$D_status) <- c("Healthy", "Future Diabetic", "Diabetic")
  names(data)[which(names(data) %in% c("age_at_collection", "abx_usage", "exclusive_bf", "delivery_type", "sex", "country", "seroconverted_time", "seroconverted_ever", "D_status"))] <- c("Age", "Antibiotics Usage", "Exclusively Breastfed", "Delivery Type", "Sex", "Country of Origin", "Seroconverted Sample", "Seroconverted Ever", "Diabetic Status")
  return(data)
}
